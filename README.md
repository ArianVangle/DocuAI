...
1. Пользователь загружает документ в формате TXT, PDF или DOCX.
2. Бэкенд извлекает текст, проверяет его на потенциально опасное содержимое и подготавливает к анализу.
3. LLM-диспетчер определяет, какой агент лучше всего подходит для запроса.
4. Выбранный агент обрабатывает документ с учётом исходного вопроса пользователя.
5. Интерфейс получает итоговый ответ вместе с информацией об этапах обработки.

| Агент | Назначение |
| --- | --- |
| `summarizer` | Формирует краткое содержание документа |
| `marketing_expert` | Создаёт отраслевые маркетинговые формулировки и варианты A/B-тестов |
| `technical_reviewer` | Отвечает на технические вопросы на основе документа |
| `document_analyst` | Выполняет глубокий анализ структуры и ключевых компонентов документа |
